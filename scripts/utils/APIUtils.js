...
const followerSchema = new Schema('followers', { idAttribute: 'fullName' });
repoSchema.define({
  owner: userSchema
});
...
export function fetchFollowers(url) {
  return fetchAndNormalize(url, followerSchema);
}

export function fetchFollowerArray(url) {
  return fetchAndNormalize(url, arrayOf(followerSchema));
}
